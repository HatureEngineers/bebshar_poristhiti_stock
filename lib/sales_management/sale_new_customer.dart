...
import 'package:path_provider/path_provider.dart'; // For creating PDF
...
class SaleNewCustomer extends StatefulWidget {
  ...
  SaleNewCustomer ({required this.appBarTitle});
  @override
...
class _SaleNewCustomerState extends State<SaleNewCustomer> {
  ...
  void _saveCustomer() async {
    ...
    try {
      // প্রথমে নাম এবং ফোন নম্বর Firestore-এ চেক করা
      final existingNameInCustomers = await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
      ...
      // .where('phone', isEqualTo: _phoneController.text)
          .get();
      if (existingNameInCustomers.docs.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('এই নামের কাস্টমার যুক্ত করা আছে'),backgroundColor: Colors.red,),
        );
        setState(() {
      ...
      if (existingPhoneNumberInCustomers.docs.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('এই নম্বর দিয়ে কাস্টমার যুক্ত করা আছে'),backgroundColor: Colors.redAccent,),//নাম এবং
        );
        setState(() {
          _isSaving = false; // Allow future submissions
        });
      ...
      if (_formKey.currentState!.validate()) {
        ...
        if (_selectedImage != null) {
          final storageRef = FirebaseStorage.instance.ref().child(
              'customer_images/${DateTime.now().millisecondsSinceEpoch}.jpg');
        ...
        // Save sales data to "sales" collection with Timestamp
        await FirebaseFirestore.instance
          ...
          'time': _selectedDate, // Save current timestamp
        });

        // Save customer data to Firestore under the user's UID with Timestamp
        final customerData = {
        ...
        // After adding the customer, create a new "history" document under this customer's document
        await customerRef.collection('history').add({
          'totalSale': transactionAmount,
          'cashPayment': cashPayment,
          'remainingAmount': customerTransaction,
        ...
        await FirebaseFirestore.instance
          ...
          'amount': cashPayment,
          'reason':
          "মোট বিক্রি: $transactionAmount & বর্তমান বাকি: $customerTransaction",
          'time': _selectedDate, // Current timestamp
        });
              ...
              children: [
                ...
                pw.Center(
                  child: pw.Text(
                    ...
                    style: pw.TextStyle(
                        font: ttf, fontSize: 14, color: PdfColors.green800),
                  ),
                ),
              ],
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'মোট বিক্রয়মূল্য/ পূর্বের বাকির পরিমাণ (টাকা)',
